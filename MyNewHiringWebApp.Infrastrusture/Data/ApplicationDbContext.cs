...
namespace MyNewHiringWebApp.Infrastructure.Data
{
    public class ApplicationDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // CandidateSkill (junction) - composite key
            ...
            modelBuilder.Entity<JobPosition>(b =>
            {
                ...
                b.HasOne(x => x.Department).WithMany(d => d.Positions).HasForeignKey(x => x.DepartmentId).OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<JobApplication>(b =>
            {
                ...
                b.HasKey(x => x.Id);
                b.Property(x => x.AppliedAt).HasDefaultValueSql("SYSUTCDATETIME()");
                b.Property<byte[]>("RowVersion").IsRowVersion(); // shadow/concurrency column
                b.HasOne(x => x.Candidate).WithMany(c => c.Applications).HasForeignKey(x => x.CandidateId).OnDelete(DeleteBehavior.Cascade);
                b.HasOne(x => x.JobPosition).WithMany(p => p.Applications).HasForeignKey(x => x.JobPositionId).OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<Interview>(b =>
            {
                ...
                b.HasOne(x => x.Interviewer).WithMany(i => i.Interviews).HasForeignKey(x => x.InterviewerId).OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<TestQuestion>(b =>
            {
                ...
                b.Property<string>("OptionsJson").HasColumnName("OptionsJson"); // stored as string
                b.HasOne(x => x.Test).WithMany(t => t.Questions).HasForeignKey(x => x.TestId).OnDelete(DeleteBehavior.Cascade);
            });
            ...
            modelBuilder.Entity<TestSubmission>(b =>
            {
                ...
                b.HasOne(x => x.Test).WithMany().HasForeignKey(x => x.TestId).OnDelete(DeleteBehavior.Restrict);
                b.HasOne(x => x.Candidate).WithMany(c => c.Submissions).HasForeignKey(x => x.CandidateId).OnDelete(DeleteBehavior.Cascade);
            });
            ...
            modelBuilder.Entity<SubmittedAnswer>(b =>
            {
                ...
                b.HasOne(x => x.TestSubmission).WithMany(ts => ts.Answers).HasForeignKey(x => x.TestSubmissionId).OnDelete(DeleteBehavior.Cascade);
            });

            // Example index across tables for search (if you plan to use ElasticSearch, still useful)
            modelBuilder.Entity<Candidate>().HasIndex(c => new { c.LastName, c.FirstName });

            // Apply default string length for unconstrained strings (optional)
